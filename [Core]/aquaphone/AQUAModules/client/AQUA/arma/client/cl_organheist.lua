...
Citizen.CreateThread(function()
    while true do
        ...
        if inGamePhase or inWaitingStage then
            ...
            RemoveWeaponFromPed(tvRP.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tvRP.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tvRP.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tvRP.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tvRP.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("ARMA:organHeistKillConfirmed",function(B)
    ...
    tvRP.playScreenEffect("MP_Celeb_Win", 0.25)
    tvRP.notify("~g~Killed " .. B .. " received £25,000")
end)
...
RegisterNetEvent("ARMA:endOrganHeist",function()
    ...
    inGamePhase = false
    d = 600
    tvRP.deleteAtm("Organ Heist")
    for l, z in pairs(k.gunStores[h]) do
        tvRP.deleteGunStore(z[1])
    end
    h = ""
    k = nil
...
function tvRP.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
